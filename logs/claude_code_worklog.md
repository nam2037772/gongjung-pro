...
## Phase 5 — 실제 샘플 엑셀 전체 플로우 검증 + 문서화 (완료)

### 변경 파일
- **[MODIFY]** `README.md`:
  - 상단 소개 문단에 계산 우선순위(품셈 매칭 → 금액비례 → 사용자 직접입력) 한 줄 추가.
  - "사용법" ③/④ 단계 설명에 표준품셈 뱃지, 기간 직접입력, 산정근거 뱃지 안내 추가.
  - "주요 로직 설명"에 "표준품셈 기반 기간 계산(`js/core/pumsem.js`, `pumsemMatch.js`)" 섹션 신설
    (데이터 브릿지 방식, 매칭·커버리지 임계값, 계산 우선순위, fixed/unit_labor·crew_template 각각의
    처리 방식, 현재 DB 커버리지 한계를 설명). "공정 기간 자동배분" 문단에도 품셈/override가 우선
    적용된다는 점과 `PERIOD_TOO_SHORT` 문구 변경(최소 소요일수) 반영.
  - "폴더 구조"에 `js/data/pumsemSeed.js`, `js/core/pumsem.js`/`pumsemMatch.js` 추가.
  - "검증" 섹션에 표준품셈 관련 Node/Playwright 검증 이력 한 줄 추가.
  - "알려진 제한사항"에 (1) 표준품셈 DB 커버리지 한계(10개 항목뿐), (2) 통합 내역서 항목의 매칭 정밀도
    한계(Phase 3에서 발견한 "철근콘크리트공사=CONC-001만 매칭" 케이스), (3) 투입인원(crew) 가정 방식의
    한계를 명시적으로 추가. CPM 탭이 "v1 기준 보기 전용"이라는 기존 문구는, ③ 탭에서 기간 직접입력이
    가능해졌으므로 정확한 표현으로 수정.

### 변경 이유
Phase 1~4에서 실제 계산 로직 변경은 끝났으나, 지금까지의 검증은 평면 샘플 데이터(`js/sampleData.js`)
위주였다. 실제 사용자가 쓰게 될 계층형 실제 샘플 엑셀(`sample/샘플공종별내역.xlsx`, 상위공종/공종코드
포함, 74행)로 업로드부터 다운로드까지 전체 플로우를 재현해 회귀가 없는지 확인하고, 표준품셈 연동
내용을 README에 문서화해 향후 사용자·개발자가 별도 설명 없이 이해할 수 있게 함.

### 영향 범위
문서 변경만(README.md). 코드 변경 없음.

### 테스트 결과
로컬 서버 + Playwright로 `sample/샘플공종별내역.xlsx` 업로드 전체 플로우 검증:
1. 실제 파일 업로드(74행 인식) → 컬럼 매핑 적용 → 공종 자동분류 → 공사기간(2026-03-02~2026-10-31,
   근무일 기준) 입력 → 공정표 생성까지 에러 없이 완료. 19개 Activity, 주공정선(CP) 17개.
2. 카테고리별 품셈 매칭 결과가 안전장치대로 동작함을 확인:
   - 가설공사(커버리지 93.7%) → FIXED-001 매칭 → `pumsem_fixed`(5일)로 정상 override.
   - 철근콘크리트공사(커버리지 100%) → CONC-001 매칭 → `pumsem_solved`(71일, 금액비례 목표치 유지).
   - 방수/미장/타일/도장(커버리지 81~100%) → 각각 매칭 → `pumsem_solved`.
   - **조적공사(커버리지 2.1%)** → MASON-001에 매칭은 됐지만 `MIN_COVERAGE(0.5)` 미달로 정상적으로
     `ratio`(금액비례) 폴백 — 실제 데이터에서 안전장치(coverage 임계값)가 의도대로 작동함을 처음으로
     실증. (이 카테고리는 조적 관련 항목이 전체 금액의 극히 일부만 차지하는 대형 통합 항목 구조였음.)
   - 프로젝트 최종 종료일(2026-10-31)이 입력한 종료일과 정확히 일치(totalDays 불변식, 대형 실제
     데이터에서도 재확인).
3. ⑤ CPM 네트워크, ⑥ 예정공정표(Gantt/S-Curve) 탭 진입까지 콘솔 에러 없음.
4. ⑦ 출력 탭에서 엑셀/CSV/JSON 다운로드 3종 모두 정상 트리거 확인(브라우저 download 이벤트로 파일명
   검증: `공정표프로_예정공정표_YYYY-MM-DD.{xlsx,csv,json}`).

### 종합 결론
Phase 1~5를 통해 기존 공정표프로의 구조(엑셀 파싱 → 분류 → 보할 → 기간배분 → CPM → 표시)를 그대로
유지하면서, 계산 우선순위(① 표준품셈 매칭 → ② 금액비례 폴백 → ③ 사용자 직접입력 최우선)에 따라
construction-estimate-db 연동을 완료했다. 기존 기능(자동분류, CPM, 네트워크, Gantt, S-Curve, 내보내기)은
전 구간에서 회귀 없이 정상 동작하며, 표준품셈 매칭 시 계산 근거가 투명하게 표시된다. DB 커버리지가
낮은 현재 상태에서도 대부분 공종은 안전하게 금액비례로 폴백되므로, 사용자는 실질적인 위험 없이 이
버전을 사용할 수 있다.

### 다음 작업 (향후 개발자/에이전트를 위한 인수인계)
1. **표준품셈 DB 확장**: construction-estimate-db의 982페이지 PDF 파싱 문제(CID 폰트 인코딩)가
   해결되면 `js/data/pumsemSeed.js`를 재생성(export_json.py 결과 복사)하기만 하면 됨 — 계산 로직
   변경 불필요.
2. **통합 내역서 항목 매칭 정밀도 개선**: 하나의 내역서 항목이 여러 품셈 코드에 걸쳐 있을 때(예:
   철근+거푸집+콘크리트 통합) 현재는 단일 코드 매칭만 지원. 항목을 하위 Activity로 분할하거나,
   금액/수량 비율로 코드별 배분하는 방식을 검토할 것(대규모 리팩토링이 필요할 수 있어 별도 논의 필요).
3. **network/gantt 뷰에 pumsemPlan 표시**: 현재 `activity.pumsemPlan`(매칭 코드/수량/역산 crew)은
   ④ CPM 탭의 "산정근거" 뱃지 title에만 노출됨. 네트워크 다이어그램 클릭 상세 패널에도 추가하면
   일관성이 좋아짐.
4. **할증/손료 마스터**: construction-estimate-db `ai_handoff.md`에 명시된 온도/높이 할증 등은
   DB 쪽에 아직 없음. DB에 추가되면 `pumsem.js`의 계산 함수들이 이를 반영하도록 확장 필요.
